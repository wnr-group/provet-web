...
// Every match is case-insensitive (`mode: "insensitive"`, i.e. ILIKE).
//
// The project runs on Postgres, where a bare `contains` is case-SENSITIVE -
// and the catalogue's names are stored in capitals ("ZYMOMAX FORTE"), so
// without this a visitor typing "zymo" found nothing at all. (This file was
// written during a spell on SQLite, whose LIKE is already case-insensitive and
// which rejects `mode` outright; if the project ever goes back to SQLite, drop
// the option here and in productNameExclusionFilter.)
const insensitive = (query) => ({ contains: query, mode: "insensitive" });

function containsAny(fields, query) {
  return { OR: fields.map((field) => ({ [field]: insensitive(query) })) };
}
...
//
// Written as `NOT: { field: insensitive }` rather than `{ not: { contains } }`
// because Prisma's nested `not` filter has no `mode` - it would negate a
// case-sensitive match while the name tier matched case-insensitively, and a
// product would land in both tiers.
function productNameExclusionFilter(query) {
  const excludes = (field) => ({ NOT: { [field]: insensitive(query) } });
