...
test("every filter matches case-insensitively", () => {
  // On Postgres a bare `contains` is case-sensitive, and product names are
  // stored in capitals - without `mode: "insensitive"`, "zymo" found nothing.
  const clauses = [
    ...productSearchFilter("x").OR,
    ...categorySearchFilter("x").OR,
    ...pageSearchFilter("x").OR,
  ];
  for (const clause of clauses) {
    const [[, condition]] = Object.entries(clause);
    assert.equal(condition.contains, "x");
    assert.equal(condition.mode, "insensitive");
  }
});
...
test("the name exclusion filter lets a NULL sku through", () => {
  ...
  // Negated with the same case-insensitive match the name tier uses, so no
  // product can land in both tiers.
  assert.deepEqual(filter.AND[0], { NOT: { name: { contains: "amox", mode: "insensitive" } } });
});
